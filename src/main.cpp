...
unsigned int nStakeMinAge = 60 * 60 * 2 * 1; // 2 hours
...
// onecoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "onecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "the crypto community will have their revenge, july 2017";
        CTransaction txNew;
        txNew.nTime = 1496360478;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1496360478;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3860205;
		if(fTestNet)
        {
            block.nNonce   = 3860205;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x595bd84a084c956e0ac067fda2362399dba532f2abfecaadc704ca247032325a"));
        block.print();
